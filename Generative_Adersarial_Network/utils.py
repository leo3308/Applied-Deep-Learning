...
def get_random_label(batch_size, hair_classes, eye_classes, face_classes, glass_classes):
    ...
    glass_code = torch.zeros(batch_size, glass_classes)
    ...
    glass_type = np.random.choice(glass_classes, batch_size)
...
def generation_by_attributes(model, device, latent_dim, hair_classes, eye_classes, face_classes, glass_classes,
    ...
    glass_tag = torch.zeros(64, glass_classes).to(device)
    
#    hair_tag = torch.zeros(144, hair_classes).to(device)
#    eye_tag = torch.zeros(144, eye_classes).to(device)
#    face_tag = torch.zeros(144, face_classes).to(device)
#    glass_tag = torch.zeros(144, glass_classes).to(device)
    ...
    for i in range(64):
        ...
        face_tag[i][face_class], glass_tag[i][glass_class] = 1, 1
    ...
    output = model(z, tag)
    if step is not None:
        file_path = '{} hair {} eyes {} face {} glass, step {}.png'.format(hair_mapping[hair_class], eye_mapping[eye_class], face_mapping, glass_mapping, step)
    else:
        file_path = '{} hair {} eyes {} face {} glass.png'.format(hair_mapping[hair_class], eye_mapping[eye_class], face_mapping[face_class], glass_mapping[glass_class])
